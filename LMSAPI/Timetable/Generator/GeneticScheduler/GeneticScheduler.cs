...
using Activity = LMSData.Activities.Activity;

namespace LMSAPI.Timetable.Generator.GeneticScheduler
{
    /// <summary>
    ...
    public class GeneticScheduler<T> : IScheduler<T> where T : IUniversityTimetable, new()
    {
        ...
        private List<Section[]> _requiredActivities = new List<Section[]>();

        private int _maxSessions = 0;

        // Represents the term that requiredSections[i] belong to                                                   
        private List<int[]> _population = new List<int[]>();
        private double[] _fitnessScores;

        // Cached SeasonalTimetable
        private Dictionary<string, T> _cachedTimetables = new Dictionary<string, T>();
        ...
        public GeneticScheduler(List<Activity> activities, TimetableScorer scorer, Preferences preferences, Restrictions restrictions)
        {
            ...
            foreach (Activity activity in activities)
            {
                ...
                _requiredActivities.Add(allowedSections.ToArray());
            }                   

            // Calculate the max number of sessions
            foreach (Section[] sessions in _requiredActivities)
            {
                if (sessions.Length > _maxSessions)
                    _maxSessions = sessions.Length;
            }
        }
        ...
        private string SerializeTable(int[] table)
        {
            ...
            foreach (int sectionIndex in table)
                serializedTable += sectionIndex.ToString().PadLeft(_maxSessions, '0');
        ...
        {
            // Store the best one
            int bestTable = GetBestTable(0, _population.Count);
            int[] temp = _population[0];
            double tempScore = _fitnessScores[0];
            _population[0] = _population[bestTable];
            _fitnessScores[0] = _fitnessScores[bestTable];
            _population[bestTable] = temp;
            _fitnessScores[bestTable] = tempScore;
            ...
            for (int i = 0; i < _population.Count; i++)
            {
                ...
                int[] parent1 = _population[parent1Index];
                int[] parent2 = _population[parent2Index];
                ...
                {
                    if (_fitnessScores[parent1Index] > _fitnessScores[parent2Index])
            ...
            for (int i = 0; i < _population.Count; i++)
                _population[i] = newGeneration[i];

            // Compute the fitness scores for each table
            for (int i = 0; i < _population.Count; i++)
                _fitnessScores[i] = GetFitnessScore(_population[i]);
        }
        ...
        private int[]? PerformOldCrossover(int[] parent1, int[] parent2)
        {
            ...
            int[] child = new int[_requiredActivities.Count];

            int randIndex = Random.Next(0, _requiredActivities.Count);
            for (int i = 0; i < randIndex; i++)
                child[i] = parent1[i];

            for (int i = randIndex; i < _requiredActivities.Count; i++)
                child[i] = parent2[i];
        ...
        {
            for (int i = 0; i < _requiredActivities.Count; i++)
            {
                if (Random.NextDouble() <= MutationRate)
                {
                    int randSession = Random.Next(0, _requiredActivities[i].Length);
                    table[i] = randSession;
                }
            }
        }
        ...
        {
            int index1 = Random.Next(0, _population.Count);
            int index2 = Random.Next(0, _population.Count);
            if (index1 < index2)
        ...
        private int GetBestTable(int leftIndex, int rightIndex)
        {
            ...
            {
                double curRank = _fitnessScores[i];
                if (curRank > bestRank)
                {
                    bestRank = curRank;
                    bestTable = i;
        ...
        {
            int[] table = new int[_requiredActivities.Count];
        ...
        private T? GetTimetable(int[] table)
        {
            ...
            if (_cachedTimetables.ContainsKey(serializedTable))
                return _cachedTimetables[serializedTable];
            ...
            for (int i = 0; i < table.Length; i++)
            {
                ...
                if (table[i] >= _requiredActivities[i].Length)
                    return default(T);

                Section section = _requiredActivities[i][table[i]];
            ...
            // Cache the timetable
            _cachedTimetables.Add(serializedTable, newTable);
        ...
        { 
            ...
            return _scorer.GetFitnessScore(timetable);
        }
        ...
        {
            _fitnessScores = new double[PopulationSize];
            ...
            for (int i = 0; i < NumGenerations; i++)
            {
                ...
                double totalScore = 0;
                foreach (int[] table in _population)
                    totalScore += GetFitnessScore(table);
                double avgScore = totalScore / _population.Count;
                ...
                double diversityPercentage = tableCount.Count / (_population.Count * 1.0);
        ...
        public List<T> GetTimetables()
        {
            ...
            for (int i = 0; i < _population.Count; i++)
            {
                int[] table = _population[i];
                if (_fitnessScores[i] > 0)
                {
                    T timetable = new T();
                    for (int j = 0; j < table.Length; j++)
                    {
                        Section section = _requiredActivities[j][table[j]];
